...
public sealed class sh_ARMarkerImageRuntimeLibraryBuilder : MonoBehaviour
{
    ...
    private IEnumerator BuildRuntimeLibraryRoutine()
    {
        ...
        {
            Debug.LogError($"{nameof(sh_ARMarkerImageRuntimeLibraryBuilder)}: 저장된 마커 조각 정보를 찾지 못했습니다. 로그인 씬에서 이미지를 다시 선택해주세요.", this);
            trackedImageManager.enabled = trackedImageManagerPreviousEnabledState;
            yield break;
        }
        ...
        {
            Debug.LogError($"{nameof(sh_ARMarkerImageRuntimeLibraryBuilder)}: 마커 조각 이미지를 텍스처로 불러오지 못했습니다.\n{markerPieceData.PiecePath}", this);
            trackedImageManager.enabled = trackedImageManagerPreviousEnabledState;
            yield break;
        }
        ...
        try
        {
            addJobState = mutableLibrary.ScheduleAddImageWithValidationJob(
                markerTexture,
                markerPieceData.TrackingImageName,
        ...
        {
            trackingResultHandler.SetTargetReferenceImageName(markerPieceData.TrackingImageName);
        }

        Debug.Log(
            $"{nameof(sh_ARMarkerImageRuntimeLibraryBuilder)}: 런타임 마커 이미지 등록 완료.\n" +
            $"마커 이름: {markerPieceData.TrackingImageName}\n" +
